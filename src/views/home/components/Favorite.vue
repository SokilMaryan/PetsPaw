    ...
    <div class="flex flex-col  rounded-[20px] bg-white ">
      <div class="flex m-5 gap-3 ">
        ...
        <router-link to="/favorite">
          <AppButton class="h-10"> FAVORITE </AppButton>
        </router-link>
      </div>

       <!-- FAVORITED -->
      <div>
        <div v-if="addFavorited.length > 0">
          <section>
            <div class="container px-5 py-5 mx-auto">
              <div class="flex flex-wrap -m-4">
                <div class="lg:w-1/3 sm:w-2/3 p-4" 
                  v-for="dog of addFavorited" 
                  :key="dog.image.id">
                  <div class="flex relative">
                    <img alt="gallery" 
                      class=" w-[200px] h-[140px] rounded-[20px] inset-0 object-cover object-center "
                      :src="dog.image.url" :id="dog.image.id" />
                    <div
                      @click="onRemove(dog)" 
                      class="hover-item"
                      >
...
function onRemove(breeds: IBreeds) {
  ...
  addHistoriAction.unshift({
    groupActionName: 'removed from Favorite',
