  ...
  <div class="flex flex-col p-8">
    ...
    <div class="flex flex-col  rounded-[20px] bg-white ">
      <div class="flex m-5 gap-3 ">
        ...
        <router-link to="/dislike">
          <AppButton class="h-10"> DISLIKED </AppButton>
        </router-link>
      </div>

      <!-- DISLIKED -->
      <div>
        <div v-if="addDisliked.length > 0">
          <section>
            <div class="container px-5 py-5 mx-auto">
              <div class="flex flex-wrap -m-4">
                <div class="lg:w-1/3 sm:w-2/3 p-4" 
                  v-for="dog of addDisliked" 
                  :key="dog.image.id">
                  <div class="flex relative">
                    ...
                    <div
                     @click="onRemove(dog)" 
                      class="hover-item "
                    >
                      <font-awesome-icon class="text-white text-xl" icon="fa-regular fa-heart" />
...
const { addHistoriAction, addDisliked } = useActionStore()

function onRemove(breeds: IBreeds) {
  for (let i = 0; i < addDisliked.length; i++) {
    if (addDisliked[i].image.id === breeds.image.id) {
      addDisliked.splice(i, 1);
      break;
    }
  }
...
<style lang="scss" scoped >
  .hover-item {
    @apply w-[200px] h-[140px] rounded-[20px] gap-2 text-base absolute z-10 flex flex-col 
           justify-center items-center pt-5 border-2 border-gray-light bg-purpur opacity-0 hover:opacity-60 cursor-pointer 
  }
</style>